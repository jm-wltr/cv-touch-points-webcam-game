...
def main():
    ...
    # Set higher resolution for webcam (add these lines)
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1920)  # Try 1920 for Full HD if your webcam supports it
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 1080)   # Try 1080 for Full HD
    ...
    tracker = MultiPersonTracker(
        track_thresh=0.5,
        match_thresh=0.6,
        reid_thresh=0.50,  # Adjusted for MediaPipe features
        track_buffer=90,
    ...
    while cap.isOpened():
        ...
        for track_id, target_info in player_targets.items():
            ...
            if target_info['touched']:
                cv2.circle(frame, target_pos, 30, color, -1)
                cv2.circle(frame, target_pos, 30, (255, 255, 255), 2)
            else:
                cv2.circle(frame, target_pos, 30, color, 3)
